...
class GeneratePDF(View):
    def get(self, request):
        ...
        list = Companyproduct.objects.filter(isdeleted=0).order_by('code')
        context = {
            "title": "Company Product Masterfile List",
            "today": timezone.now(),
            "company": company,
            "list": list,
            "username": request.user,
        }
        return Render.render('companyproduct/list.html', context)
