...
class GeneratePDF(View):
    def get(self, request):
        ...
        list = Maingroupproduct.objects.filter(isdeleted=0).order_by('code')
        context = {
            "title": "Main Group Product Masterfile List",
            "today": timezone.now(),
            "company": company,
            "list": list,
            "username": request.user,
        }
        return Render.render('maingroupproduct/list.html', context)
