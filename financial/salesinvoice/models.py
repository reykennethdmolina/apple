...
class Simain(models.Model):
    ...
    jvmain = models.ForeignKey('journalvoucher.Jvmain', related_name='simain_jvmain_id')
...
class Sidetailtemp(models.Model):
    ...
    supplierbreakstatus = models.IntegerField(blank=True, null=True)
    employeebreakstatus = models.IntegerField(blank=True, null=True)
    isautogenerated = models.IntegerField(blank=True, null=True)
...
class Silogs(models.Model):
    user = models.ForeignKey(User, related_name='silogs_user_id')
    ...
    def get_absolute_url(self):
        return reverse('silogs:detail', kwargs={'pk': self.pk})
